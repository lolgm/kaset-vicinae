...
- The Raycast extension this was ported from had a menu bar command. Vicinae
  supports only `view` and `no-view` commands, so there is no equivalent here —
  use *Now Playing* instead.
